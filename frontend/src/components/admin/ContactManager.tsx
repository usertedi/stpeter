      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          <button
            ...
            className={`px-3 py-1 text-sm rounded-md ${filter === 'all' ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700'}`}
          >
            All
          </button>
          <button
            onClick={() => setFilter('unread')}
            className={`px-3 py-1 text-sm rounded-md ${filter === 'unread' ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700'}`}
          >
            Unread
          </button>
          <button
            onClick={() => setFilter('read')}
            className={`px-3 py-1 text-sm rounded-md ${filter === 'read' ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700'}`}
          >
            Read
          </button>
        </div>
              ...
              {filteredMessages.length === 0 ? (
                ...
                filteredMessages.map((message) => (
                  <div
                    ...
                    className={`p-4 cursor-pointer hover:bg-gray-50 ${selectedMessage?.id === message.id ? 'bg-gray-100' : ''} ${!message.read ? 'border-l-4 border-primary-600' : ''}`}
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex items-center">
                        ...
                        <span className={`font-medium ${!message.read ? 'text-gray-900' : 'text-gray-700'}`}>
                          {message.name}
                        </span>
                      </div>
                    ...
                    <h3 className={`mt-1 text-sm ${!message.read ? 'font-semibold' : 'font-normal'}`}>
                      {message.subject}
                    </h3>
